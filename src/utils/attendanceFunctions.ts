...
// یک تحمل کوچک (به دقیقه) برای مقایسه‌ی «مرخصیِ کامل روز»؛ این جلوی رد شدن
// موارد سالم به خاطر گرد شدنِ ثانیه/میلی‌ثانیه در محاسبات moment را می‌گیرد.
const FULL_DAY_LEAVE_TOLERANCE_MINUTES = 2;
  ...
  for (const day of filteredCalendar) {
    ...
    if (attendance && attendance.sessions.length > 0) {
      ...
      if (firstCheckIn && lastCheckOut) {
        ...
        if (isFullDayCovered(plannedDuration, leaveMinutes)) {
          report.push({
            date: dateStr,
            shamsiDate,
            expectedStart: plannedStart.format("HH:mm"),
      ...
      const isFullDayLeave =
        ...
        isFullDayCovered(plannedDuration, leaveMinutes);
      ...
      if (isFullDayLeave) {
        ...
        report.push({
          ...
          expectedStart: plannedStart.format("HH:mm"),
          expectedEnd: plannedEnd.format("HH:mm"),
          leaveMinutes: leaveMinutes > 0 ? minutesToTime(leaveMinutes) : undefined,
          status: "absent",
        });
      } else {
        report.push({
...
// آیا مرخصیِ آن روز، کل بازه‌ی برنامه‌ریزی‌شده‌ی شیفت را پوشش می‌دهد؟
// (با کمی تحمل، تا اختلاف چند دقیقه‌ای ناشیِ از گرد شدن، باعث رد شدن اشتباهی نشود)
function isFullDayCovered(plannedDuration: number, leaveMinutes: number): boolean {
  if (plannedDuration <= 0) return false;
  return leaveMinutes >= plannedDuration - FULL_DAY_LEAVE_TOLERANCE_MINUTES;
}
